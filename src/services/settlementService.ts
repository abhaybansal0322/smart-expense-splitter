...
export async function computeGroupBalances(groupId: string): Promise<UserBalance[]> {
  const { rows } = await query<any>(
    `WITH member_ids AS (
     ...
     SELECT
       m.id AS user_id, m.name, m.email, m.upi_id,
       COALESCE(p.total_paid, 0)::float AS paid,
       COALESCE(o.total_owed, 0)::float AS owed,
       COALESCE(cs.total_sent, 0)::float AS sent,
       COALESCE(cr.total_received, 0)::float AS received,
       ROUND(
  ...
  console.log(`[DEBUG] Balances for group ${groupId}:`, rows.map(r => ({
    name: r.name,
    paid: r.paid,
    owed: r.owed,
    sent: r.sent,
